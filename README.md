...
- **橡皮有两种**，在橡皮的面板里选（普通工具栏和笔具盘是同一个设置）：
  - **对象橡皮擦**（默认）：碰到哪一笔就整笔删掉。
  - **像素橡皮擦**：把扫过的那一段从笔画里切掉，两头留下来。笔迹仍然是矢量的——
    它不是往位图上抹，而是把一条笔画换成切剩的几段，所以同步、撤销、导出 PDF
    一概不用改格式，发出去的就是一条 `remove` 加一条 `restore`，切出来的段沿用
    原来那条的层叠序号，叠放关系不变。代价是笔画数量会涨，切一刀多一条。
    比橡皮粗的笔会被整条切断而不是啃掉一半，切笔画这个做法只能整个截面一起断。
  大小不随笔身角度或者快慢变，默认就是最细那一档（直径 6），五档到 50 为止。
  一次拖动算一次撤销，撤回去就是原来那几条完整的笔画。
