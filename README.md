...
局域网里任何人都能打开这个地址开始写字——这是它的用法。但**换白板、建板删板、
改名、改背景、改存储目录、导入导出、检查更新这些一律只有本机能做**，默认如此。

- 别的设备不管怎么访问，拿到的都是书写界面，`?role=mac` 也没用。
- 判断依据是 TCP 对端地址（回环，或者本机自己的局域网地址），不是浏览器
  User-Agent，也不是 WebSocket 握手里那个 role——那两样客户端想填什么填什么。
  权限在连接建立的那一刻就定死了，之后改不动。
- 只有 Mac 界面用得到的那几个接口（`/api/info`、`/api/boards`、`/api/thumb`、
  `/api/doc` 上传、`/api/export`）对外部设备直接 403；写字要用的
  （页面、WebSocket、文档分页图、描述文件、诊断上报）照常。
- 检查更新根本不经过 HTTP，它走的是 pywebview 的本地接口，只有那个窗口里有。
- 真要让别的设备也能管，在 Mac 的**白板设置**里打开「允许其他设备控制」。
  这个开关记在配置文件的 `allow_remote_control` 里，默认是关的。
