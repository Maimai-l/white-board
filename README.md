...
- **笔具盘（beta）**：在颜色面板里勾上「笔具盘 beta」就换成另一套工具栏——参考 iPad
  自带的 PencilKit 工具面板，按 Material 3 重画：
  - 四件工具画成四支立着的笔，笔尖就是这支笔自己的墨色，选中的那支抬起来；
  - **每支笔各记各的颜色和粗细**，换笔不会把上一支的设置带过去；
  - 点正在用的那支笔弹出它的设置（粗细、颜色），不再需要单独的颜色按钮；
  - 左端有一条握把，按住可以把整条栏拖到上下左右任意一边；
  - 拖进屏幕角落会缩成一个圆，圆上显示当前那支笔，写字时不挡画面，点一下再展开。
