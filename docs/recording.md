...
在 iPad 的地址后面加上 `?record=1`，页面底部会出现一个小面板：

```
http://<Mac 的地址>:8000/?role=ipad&record=1
```

点「开始录制」，把出问题的操作做一遍，点「停止录制」。录像会直接 POST 到 Mac
上，存在数据目录的 `recordings/` 里，文件名是时间戳。面板上会显示存到了哪。

服务端存不下（比如页面是从别处打开的）就退回浏览器下载。

这个面板只有带上 `?record=1` 才挂出来，正式界面上不会多一个按钮。
