...
- **PDF**：笔迹编成一段新的内容流，以 `/Contents` 数组的形式追加在原页面后面，
  原有的内容流对象一个字节都不重写。笔迹先做 RDP 抽稀（0.35 pt），坐标按 1/4 pt
  取整后靠 `cm` 缩放回去（整数比小数好压），不透明的笔按 0.5 pt 分桶描成折线，
  半透明的荧光笔仍然是一次填充闭合轮廓（重叠处才不会变深），最后整段 zlib 压缩。
  实测每一笔约 110 字节。代码在 `whiteboard/inkpdf.py`。
